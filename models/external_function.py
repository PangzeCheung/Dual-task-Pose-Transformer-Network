...
class AffinityLoss(nn.Module):
    ...
    def comput_affinity(self, x_vgg, y_vgg, source_vgg):
        ...
        y_query = y_vgg.view(input_shape[0], -1, input_shape[2] * input_shape[3]).permute(0, 2, 1)  # B X N X C

        source_key = source_vgg.view(input_shape[0], -1, input_shape[2] * input_shape[3]).permute(0, 2, 1)  # B X N X C

        escape_NaN = torch.FloatTensor([1e-4])
        escape_NaN = escape_NaN.cuda()

        max_x = torch.max(torch.sqrt(reduce_sum(torch.pow(x_query, 2), axis=[2], keepdim=True)), escape_NaN)
        max_y = torch.max(torch.sqrt(reduce_sum(torch.pow(y_query, 2), axis=[2], keepdim=True)), escape_NaN)
        max_source = torch.max(torch.sqrt(reduce_sum(torch.pow(source_key, 2), axis=[2], keepdim=True)), escape_NaN)
        ...
        y_attention = self.softmax(y_energy)  # BX (N) X (N)
