...
class Visualizer():
    ...
    def print_current_errors(self, epoch, iter, i, errors, lr_G, lr_D, t):
        message = '(epoch: %d, iters: %d, total iters: %d, time: %.3f) ' % (epoch, iter, i, t)
        for k, v in errors.items():
            message += '%s: %.3f ' % (k, v)
        message += 'learning_rate_g: %.10f' % lr_G
        message += ' learning_rate_d: %.10f' % lr_D
        print(message)
        with open(self.log_name, "a") as log_file:
            log_file.write('%s\n' % message)
